...
function funcion_usuario($options)
{
	...
	if (false != $onlines) {
		...
		for ($i = 0; $i < $total; $i++) {
			...
			$miembros .= '<tr><td><img src="'.XOOPS_URL.'/modules/ntxusuario/images/banderas/'.$bandera.'.gif"></td><td><font size="1"><a href="'.XOOPS_URL.'/userinfo.php?uid='.$onlines[$i]['online_uid'].'">'.$onlines[$i]['online_uname'].'</a></td><td><font size="1">'.$onlineUsers[$i]['module'].'</td></tr>';
